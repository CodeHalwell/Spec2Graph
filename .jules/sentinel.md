...
## 2026-05-22 - Denial of Service via Memory Exhaustion in Formula Parsing
**Vulnerability:** The `parse_formula` function in `spec2graph/eval/decode.py` parses chemical formulas using regex (`_FORMULA_TOKEN.findall(formula)`) and directly creates a counts dictionary. Downstream code then uses `[element] * count` to expand these. Missing limits on both string length and the parsed atom count meant an excessively long string (e.g. `C` repeated thousands of times) or an extremely high count multiplier (e.g. `C1000000000`) could cause significant CPU spinning or interpreter crashes via Out of Memory (OOM) errors during the list expansion.
**Learning:** Functions translating string-based user input into potentially vast memory allocations must explicitly bound both the input string length and the decoded integer quantities. Even simple logic loops are dangerous when input data cardinality isn't strictly controlled.
**Prevention:** Enforce a hard length limit on user input immediately at the function boundary (e.g. `len(formula) > 2000`) and cap any parsed integer logic that drives list allocations to safe maximums (e.g. `count > 10000`).
