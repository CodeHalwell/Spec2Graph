...
## 2024-05-26 - Prevent memory exhaustion from large JSONL reads
**Vulnerability:** Found `.read_text().splitlines()` used on an externally provided `jsonl_path` in `spec2graph/eval/benchmark.py`. This buffers the entire stringified file content and list of strings in memory. If a malicious user supplies an abnormally large or deeply nested JSONL file, this could cause an Out-Of-Memory (OOM) crash resulting in a Denial of Service (DoS).
**Learning:** Functions that parse entire external log or artifact files natively in memory using `.read_text()` or similar synchronous whole-file load mechanisms are vulnerable to resource exhaustion.
**Prevention:** Instead of reading the entire file into memory with `.read_text().splitlines()`, stream the file line-by-line using a context manager and iterator (`with open(path, "r", encoding="utf-8") as f: for line in f:`). Always explicitly declare `encoding="utf-8"` to prevent OS-specific locale decoding bugs (like on Windows).
